...
spark = SparkSession.builder \
    .appName("atm-visit-updates") \
    .getOrCreate()

# TO-DO: set the log level to WARN
spark.sparkContext.setLogLevel("WARN")

# TO-DO: read the atm-visits kafka topic as a source into a streaming dataframe with the bootstrap server
...
kafkaSQLDF = spark.sql("select * from ATMVisits")

# TO-DO: write the dataFrame from the last select statement to kafka to the atm-visit-updates topic, on the broker
# localhost:9092, and configure it to retrieve the earliest messages

kafkaSQLDF.writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "kafka:19092") \
    .option("topic", "atm-visit-updates") \
    .option("checkpointLocation", "/tmp/checkpoint-atm-visits-updates") \
